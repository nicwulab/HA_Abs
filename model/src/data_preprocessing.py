...
#     assuming classes = {'stem', 'head', 'others'}, num_sample of -ve cases being average of 'stem' and 'head'
    ...
    # false cases
    if num_sample < cleaned_df.shape[0]:
...
def encode(X_df, y_df, cdr_char, test_size, pad=30, equal_valtest=False, le=True, col_names={'id':'Id','sequence':['VH_AA','VL_AA']}, merge=True):
...
def sample_equal_ratio(x, y, le, single=True):
    ...
    if flase_mask_id.shape[0] <= true_cnt:
        return x, y

    negative_samples = np.random.choice(flase_mask_id, true_cnt, replace=False)
