...
def train_dl(CDR_model, train, val):
    ...
    # EVAL_HEADERS = ['CategoricalAccuracy', 'Precision', 'Recall', 'ROC-AUC', 'PRC-AUC']
    ...
    CDR_model.compile(
        ...
        metrics=METRICS,
#         metrics=['categorical_accuracy', f1_score, precision_m, recall_m]
    )
...
def train_dl_multi(CDR_model, train, val):
    METRICS = [
        CategoricalAccuracy(name='accuracy'),
        Precision(name='precision'),
        Recall(name='recall'),
...
def test_model(model, data):
    ...
    pred_bool = pred>thre
    label = y_test#>thre#np.zeros(pred_bool.shape)>thre
    acc = (pred_bool == label).reset_index(drop=True)
    
    eval_df = pd.DataFrame(
            {'Id':X_test_id.reset_index(drop=True)[~acc], 
             'Label': label.reset_index(drop=True)[~acc],
...
def plot_cm(labels, predictions, p=0.5, relative=False, annot=True):
    ...
    fmt = '.2g' if relative else 'd'
    
    plt.figure(figsize=(5,5))
    sns.heatmap(con_mat, annot=annot, fmt=fmt)
...
def plot_cm_multi(labels, y_pred_cate, le, p=0.5, num_classes=3, relative=False, annot=True):
    ...
    sns.heatmap(con_mat_df, annot=annot, fmt="d")#,cmap=plt.cm.Blues)
    plt.tight_layout()
