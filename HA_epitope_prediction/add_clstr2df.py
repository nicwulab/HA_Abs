...
def add_clstr2df(cluster_identity):
    ...
    metadata_DF = pd.read_excel('result/epitope_info_clstr_v1.xlsx')
    cluster_df = pd.read_csv('result/cluster.tsv', sep='\t')
    DF=pd.merge(metadata_DF,cluster_df,on='Name')
    DF.to_excel('result/epitope_info_clstr_v1.xlsx',index=False)
    os.remove('result/cluster.tsv')

if __name__ == "__main__":
    # Set up argparse
    parser = argparse.ArgumentParser(description='Add cluster to an Excel table')
    parser.add_argument('-i','--input_file', default='data/epitope_info_v1.xlsx', type=str, help='the input Excel file')
    parser.add_argument('-o','--output_file',  default='result/epitope_info_clstr_v1.xlsx',type=str, help='the output xlsx file')
